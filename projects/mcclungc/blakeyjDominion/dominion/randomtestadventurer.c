...
//TO ZERO IN ON BUGS IN TEAMMATE'S CODE, I REWROTE SLIGHTLY TO BE MORE VERBOSE IN OUTPUT WHEN ERRORS ARE FOUND
//(INCLUDING PRINTING THE TREASURE COUNT FOR EACH PLAYER PILE AFTER CARD EFFECT CALLED
...
void randomTestAdventurer(int player, struct gameState *randomStatePost) {
	...
	int postDeckTreasure = 0;
	int postHandTreasure = 0;
	int postDiscardTreasure = 0;
	...
	//UNCOMMENT THE FOLLOWING 3 LINES FOR MORE VERBOSE OUTPUT
	...
	//count treasure cards in player's hand and deck AFTER running card effect
	//so you can tell correct treasure amount was added to hand 
	for (i = 0; i < randomStatePost->handCount[player];i++)
	{
		if (randomStatePost->hand[player][i] == silver || randomStatePost->hand[player][i] == gold ||
	...
	//UNCOMMENT THE FOLLOWING 3 LINES FOR MORE VERBOSE OUTPUT
	//printf("After call to adventurer card effect, player has %d treasure cards in hand\n", postHandTreasure);
	//printf("After call to adventurer card effect, player has %d treasure cards in deck\n", postDeckTreasure);
	//printf("After call to adventurer card effect, player has %d treasure cards in discard pile\n", postDiscardTreasure);
...
int main() {
	...
	SelectStream(2);
	PutSeed(3);
	int numIterations = 100;//ADJUST THIS NUMBER TO SET NUMBER OF TEST ITERATIONS

	for (n = 0; n < numIterations; n++) {
		//UNCOMMENT THE FOLLOWING LINE TO SHOW WHAT ITERATION YOU ARE ON
		//printf("n = %d\n", n);
